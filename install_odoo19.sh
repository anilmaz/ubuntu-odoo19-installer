...
echo -e "\n${CYAN}${BOLD}Step 1: Synchronizing Core Apt Distribution Repositories${NC}"
apt-get update -y >/dev/null 2>&1
show_progress 1.5 "Updating package manifests"

echo -e "\n${CYAN}${BOLD}Step 2: Installing Libraries and Data Runtimes${NC}"
apt-get install -y python3-pip python3-dev python3-venv python3.12-venv libxml2-dev libxslt1-dev \
    ...
    git nodejs npm xfonts-75dpi curl libaio1t64 wkhtmltopdf postgresql postgresql-client bc >/dev/null 2>&1
...
show_progress 1.2 "Configuring PostgreSQL cluster structures with Superuser Clearance"
